...
def grad(v_hat, ikx, iky):
    ...
    dvx = np.real(np.fft.ifftn(ikx * v_hat))
    dvy = np.real(np.fft.ifftn(iky * v_hat))
    return dvx, dvy
...
def curl(vx_hat, vy_hat, ikx, iky):
    ...
    return np.real(np.fft.ifftn(ikx * vy_hat - iky * vx_hat))
...
def main():
    ...
    for i in range(Nt):
        ...
        # Rewritten to vorticity formulation
        # vorticity
